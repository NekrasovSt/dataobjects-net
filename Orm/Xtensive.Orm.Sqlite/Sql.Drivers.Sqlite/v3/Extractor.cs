  ...
  internal class Extractor : Model.Extractor
  {
    ...
    private void ExtractForeignKeys()
    {
      ...
      foreach (var table in schema.Tables) {
        ...
        using (IDataReader reader = cmd.ExecuteReader()) {
          while (reader.Read()) {
            int columnPosition = ReadInt(reader, 1);
            if (columnPosition <= lastColumnPosition) {
              localTable = table;
              var foreignKeyName = String.Format(CultureInfo.InvariantCulture, "FK_{0}_{1}", localTable.Name, ReadStringOrNull(reader, 2));
              foreignKey = localTable.CreateForeignKey(foreignKeyName);

              ReadCascadeAction(foreignKey, reader, 6);
              var referencedSchema = table.Schema; //Schema same as current
              foreignTable = referencedSchema.Tables[ReadStringOrNull(reader, 2)];
              foreignKey.ReferencedTable = foreignTable;
            }
            var localColumn = localTable.TableColumns[reader.GetString(3)];
            var foreignColumn = foreignTable.TableColumns[reader.GetString(4)];
            foreignKey.Columns.Add(localColumn);
            foreignKey.ReferencedColumns.Add(foreignColumn);
